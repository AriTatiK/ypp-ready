...
export default async function AfdbRealRunPage({
  params,
}: {
  params: Promise<{ sessionId: string }>;
}) {
  const { sessionId } = await params;
  const user = await requireUser(`/assessment/afdb-real/run/${sessionId}`);
  ...
  if (session.status === "completed") {
    redirect(`/assessment/afdb-real/results/${session.id}`);
  }
  ...
  return (
    <div className="px-4 py-10 sm:px-6 sm:py-14">
      ...
      <QuizRunner
        ...
        context="afdb_real"
        timed={true}
        timeLimitSeconds={session.time_limit_seconds ?? 2700}
        resultsHref={`/assessment/afdb-real/results/${session.id}`}
      />
    </div>
  );
}
