...
export default async function MockResultsPage({
  ...
  const user = await requireUser(`/assessment/mock/results/${sessionId}`);
  ...
  if (!session || session.user_id !== user.id || session.type !== "mock") {
    return (
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-10 sm:px-6 sm:py-14">
      ...
      <h1 className="text-2xl font-semibold tracking-tight text-ink sm:text-3xl">{assessment.resultsTitle}</h1>
