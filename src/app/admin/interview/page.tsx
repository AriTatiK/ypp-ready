...
import { deleteInterviewAction } from "@/app/actions/admin";

export const metadata = { title: "Interview Questions — Admin — YPPReady" };
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-10 sm:px-6 sm:py-14">
      ...
      <div className="mt-6 flex flex-wrap items-center justify-between gap-4">
        <h1 className="text-2xl font-semibold tracking-tight text-ink">{dict.admin.interviewTab}</h1>
        <Button href="/admin/interview/new" variant="accent" size="sm">
          {dict.admin.addNew}
        </Button>
      </div>
            ...
            <tbody>
              {questions.map((q) => (
                <tr key={q.id} className="border-b border-border-subtle/60">
                  ...
                  <td className="py-2 pr-4 text-ink">{q.org_scope}</td>
                  ...
                  <td className="py-2 pr-0 text-right">
                    <div className="flex items-center justify-end gap-4">
                      <Link
                        href={`/admin/interview/${q.id}/edit`}
